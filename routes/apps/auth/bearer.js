...
router.post('/me',
  ...
  [
    ...
    check('email').not().isEmpty()
      ...
      .not()
      .custom((val, { req }) => Customer.existEmail(val, req.user.id))
...
router.put(
  ...
  nextCatch(async (req, res, next) => {
    ...
    await Customer.revokeRefreshToken(refresh_token);
...
router.post(
  ...
  nextCatch(async (req, res, next) => {
    const { refresh_token } = req.body;
    const user = await Customer.findByRefreshToken(refresh_token);
